...
contract BuyableTokenWithRefund is IERC20{
    ...
    function setRate(uint256  _tokenRate) public onlyOwnerOrDelegate returns (uint8) {
    ...
    struct BuyDetails {
        uint256 buyTime;
        uint256 tokenUnitsToBuy; // shows the smallest units of token bought 
    ...
    function buy(address payable buyer) public payable virtual returns (bool) {
        ...
        //uint256 tokensToBuy = (msg.value * tokenRate)/decimals;// if needed, calculates the no. tokens bought
       
        uint256 weiReceived = msg.value; // wei paid by the buyer
       
        uint256 tokenUnitsToBuy = (msg.value*tokenRate); // calculates the smallest units of token bought
       
        uint256 tokenBuyingRate = tokenRate; // the rate at which the purchase was made
        
        uint256 tokensLeft = tokenUnitsToBuy; // tokens left after any refund
      
      _balances[buyer] =  _balances[buyer].add(tokenUnitsToBuy); // adds the tokens bought to buyer balance
      
      _balances[owner] =  _balances[owner].sub(tokenUnitsToBuy); // subtracts the tokens from the owner balance
      ...
      _balances[weiReceipt] =  _balances[weiReceipt].add(msg.value); //adds the wei received to the above defined weiReceipt address
        ...
        buyerDetails[buyer] = BuyDetails(buyTime, tokenUnitsToBuy, weiReceived, tokenBuyingRate, tokensLeft);
 ...
 //  }
   ...
   function refundCalc (address payable refundee, uint256 tokenRefund) public view returns (uint256) {
    uint256 y= (tokenRate * tokenRefund * buyerDetails[refundee].weiReceived) / (buyerDetails[refundee].tokenBuyingRate* buyerDetails[refundee].tokenUnitsToBuy); 
       return y;
   }
   
   // function to transfer the wei calculated above to the refundee 
    function weiTransfer (address recipient, uint256 amount) public virtual {
        
        address sender = weiReceipt;
  ...
  // function to refund wei against current token rate; uses refundCalc, transfer and weiTransfer functions within
    function refund(address payable refundee , uint256 tokenRefund ) public payable onlyOwnerOrDelegate {
        require (msg.sender == refundee);
        
        // to ensure refund time falls within 1 month of refund period requirement
        require ( now <= (buyerDetails[refundee].buyTime + 2629743), "Purchase time has surpassed 1 month");
